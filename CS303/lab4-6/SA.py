...
def sa():
    ...
    T = 50  # initiate temperature
    Tmin = 10  # minimum value of terperature

    param = Parameter.fromJson("6.json")
    ...
    while T >= Tmin:
        ...
        for i in range(k):
            ...
            new_param = Parameter(
                param.r +
                np.random.uniform(low=-0.000000000005, high=0.000000000005) * T,
                param.l +
                np.random.uniform(low=-0.000000000005, high=0.000000000005) * T,
                param.e, param.n
            )
            ...
            if new_param.check():
                ...
                if new_param.val < param.val:
                    ...
                    p = exp(-1e4 * (new_param.val - param.val) / T)
    ...
    best = Parameter.fromJson("12.json")
